...
| Standard persistent tmux | Supported, with one gap | App-created session, bounded history, reconnect/reattach. Availability depends on tmux on the host. **Capture-based history recovery is not implemented in this app** — after a reattach, scrolling up shows only what tmux replayed, not the pane's earlier history. |
