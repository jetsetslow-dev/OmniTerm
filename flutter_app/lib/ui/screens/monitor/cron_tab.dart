...
class _CronTabState extends State<CronTab> {
  ...
  Widget _body(BuildContext context, MonitorViewModel vm, ColorScheme scheme) {
    ...
    if (!vm.cronReadable) {
      ...
      // Scrollable, because this sits inside an `Expanded` and its height is not the app's to
      // choose: the host's refusal is quoted verbatim, and a long one at 200% text in landscape
      // overflowed the pane. An explanation the user cannot finish reading is not an explanation.
