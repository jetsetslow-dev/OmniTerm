        ...
        child: Row(
          children: [
            ...
            InkWell(
              ...
              onTap: () => _showHealthBreakdown(context, vm, server),
...
/// Explains the monitored host's health score, ported from `HealthBreakdownDialog` in `ui/AppUi.kt`.
///
/// Reads the breakdown from the view model rather than recomputing it, so the explanation is
/// produced by the same config and the same readings that produced the number.
Future<void> _showHealthBreakdown(BuildContext context, MonitorViewModel vm, Server server) async {
  final breakdown = vm.healthBreakdown;
  ...
  await showDialog<void>(
    ...
    builder: (context) {
      ...
      return AlertDialog(
        key: const ValueKey('monitor.health.dialog'),
        title: Text('Health score · ${HostDisplay.instance.name(server)}'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            Text(
              ...
              key: const ValueKey('monitor.health.score'),
            ...
            if (breakdown.offline)
              const Text(
                ...
                key: ValueKey('monitor.health.offline'),
              ...
              const Text(
                ...
                key: ValueKey('monitor.health.healthy'),
                style: TextStyle(fontSize: 14, color: OmniColors.green),
              )
            else ...[
              const Text('Starting from 100, these readings were deducted:', style: TextStyle(fontSize: 14)),
              const SizedBox(height: 6),
              for (final factor in breakdown.factors)
                Padding(
                  key: ValueKey('monitor.health.factor.${breakdown.factors.indexOf(factor)}'),
        ...
        actions: [
          TextButton(
            key: const ValueKey('monitor.health.close'),
