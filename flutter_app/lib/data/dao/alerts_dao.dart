...
class AlertsDao extends DatabaseAccessor<AppDatabase> with _$AlertsDaoMixin {
  ...
  Future<void> deleteRuleById(int id) =>
      (delete(alertRules)..where((r) => r.id.equals(id))).go();

  Future<void> deleteRulesExceptServers(List<int> keepServerIds) =>
      (delete(alertRules)
            ..where((r) => r.serverId.equals(0).not() & r.serverId.isNotIn(keepServerIds)))
          .go();
  ...
  Stream<List<ActiveAlert>> watchActiveAlerts() => (select(activeAlerts)
        ..orderBy([(a) => OrderingTerm.desc(a.triggeredTime)]))
      .watch();

  Future<List<ActiveAlert>> getActiveAlerts() => (select(activeAlerts)
        ..orderBy([(a) => OrderingTerm.desc(a.triggeredTime)]))
      .get();
  ...
  Future<void> deleteAlert(int id) =>
      (delete(activeAlerts)..where((a) => a.id.equals(id))).go();

  Future<void> setAcknowledged(int id, bool acknowledged) =>
      (update(activeAlerts)..where((a) => a.id.equals(id)))
          .write(ActiveAlertsCompanion(acknowledged: Value(acknowledged)));
  ...
  Future<void> muteAlert(int id, int mutedUntil) =>
      (update(activeAlerts)..where((a) => a.id.equals(id)))
          .write(ActiveAlertsCompanion(mutedUntil: Value(mutedUntil)));

  Future<void> deleteAlertsExceptServers(List<int> keepServerIds) =>
      (delete(activeAlerts)
            ..where((a) => a.serverId.equals(0).not() & a.serverId.isNotIn(keepServerIds)))
          .go();
  ...
  Stream<List<AlertHistoryRow>> watchAlertHistory() => (select(alertHistory)
        ..orderBy([(h) => OrderingTerm.desc(h.historyTime)]))
      .watch();

  Future<List<AlertHistoryRow>> getAlertHistory() => (select(alertHistory)
        ..orderBy([(h) => OrderingTerm.desc(h.historyTime)]))
      .get();
  ...
  Future<void> deleteHistoryExceptServers(List<int> keepServerIds) =>
      (delete(alertHistory)
            ..where((h) => h.serverId.equals(0).not() & h.serverId.isNotIn(keepServerIds)))
          .go();
